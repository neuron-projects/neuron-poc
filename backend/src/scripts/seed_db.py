...
def seed_database():
    print(f"Connecting to DynamoDB table: {settings.DYNAMODB_TABLE}")
    ...
    print(f"Seeding {len(mock_incidents)} items...")

    with table.batch_writer() as batch:
        for data in mock_incidents:
            # Validate with Pydantic model to ensure keys match exactly
            incident = Incident(**data)
            # Dump to dict, preserving enum values
